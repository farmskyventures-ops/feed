-- =====================================================================
-- Farmsky demo seed data — PostgreSQL (passwords plain text for DEMO ONLY)
-- Uses ON CONFLICT DO NOTHING so re-running is safe, then resyncs the
-- SERIAL sequences so subsequent auto-id inserts don't collide.
-- =====================================================================

INSERT INTO users (id, full_name, phone, email, password, role, status, region) VALUES
...
INSERT INTO agents (id, user_id, region, permissions, commission_rate) VALUES
...
INSERT INTO products (id, sku, name, category, supplier_id, buying_price, cash_markup_pct, credit_markup_pct, cash_price, credit_price, quantity, unit, reorder_threshold) VALUES
...
INSERT INTO customers (id, user_id, agent_id, full_name, national_id, date_of_birth, gender, mobile, county, sub_county, ward, village, latitude, longitude, value_chain_type, value_chain, acreage, herd_size, farm_experience, kyc_status, risk_band, credit_score) VALUES
...
INSERT INTO customers (id, agent_id, full_name, national_id, gender, mobile, county, value_chain_type, value_chain, kyc_status, risk_band, credit_score) VALUES
  (2, 2, 'Peter Kamau', '31245678', 'Male', '+254790112233', 'Nakuru', 'crop', 'Maize', 'pending', 'medium', 610)
ON CONFLICT DO NOTHING;

-- Sample completed cash sale + an active credit contract
INSERT INTO murabaha_contracts (id, contract_ref, customer_id, agent_id, product_id, quantity, payment_type, supplier_cost, markup_pct, murabaha_price, term_months, monthly_payment, delivery_location, status, ownership_recorded, consent_given, amount_paid, outstanding) VALUES
...
INSERT INTO repayments (contract_id, installment_no, due_date, amount_due, amount_paid, status, paid_at) VALUES
...
INSERT INTO invoices (invoice_ref, contract_id, customer_id, amount, status) VALUES
  ('INV-2026-0001', 1, 1, 5500, 'paid'),
  ('INV-2026-0002', 2, 1, 12000, 'partial')
ON CONFLICT DO NOTHING;

INSERT INTO transactions (txn_ref, contract_id, customer_id, amount, method, type, mpesa_receipt, status) VALUES
...
-- Resync SERIAL sequences after explicit-id inserts so future auto-ids don't collide.
SELECT setval(pg_get_serial_sequence('users', 'id'), GREATEST((SELECT MAX(id) FROM users), 1));
SELECT setval(pg_get_serial_sequence('agents', 'id'), GREATEST((SELECT MAX(id) FROM agents), 1));
...
SELECT setval(pg_get_serial_sequence('murabaha_contracts', 'id'), GREATEST((SELECT MAX(id) FROM murabaha_contracts), 1));
SELECT setval(pg_get_serial_sequence('repayments', 'id'), GREATEST((SELECT MAX(id) FROM repayments), 1));
